...
class Axi {
    ...
    setAnimationOpts(opts) {
        ...
        if (typeof this.animationOpts.duration === 'function')
            this.animationOpts.duration = this.animationOpts.duration();
    }
    setReversed() {
    ...
    parseTweens(target, type, opts, idx, targetLen) {
        ...
        return vals.map((d, i) => {
            ...
            if (typeof fromVal === 'function')
                fromVal = fromVal(target, idx, targetLen);
            if (typeof toVal === 'function')
                toVal = toVal(target, idx, targetLen);
            let from, to;
            if (isColor) {
